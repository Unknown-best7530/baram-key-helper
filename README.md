...
- 키보드 및 마우스 입력 매핑
  - 하나의 매핑에 여러 키·조합키·마우스 입력을 추가해 같은 출력 설정 가능
  - 복합 출력, 횟수별 출력, 마우스 출력 설정 지원
- Shift / Ctrl / Alt 조합 입력 딜레이 설정
- `기타 설정`의 `입력 키는 출력되지 않음` 설정 지원
  - 매핑 커스텀이 ON일 때, 타이머 창이 열려 있을 때, 주기 커스텀이 실행 중인 입력 대기·조건에서 사용한 키보드 입력은 게임에 전달하지 않음
  - 마우스 클릭·휠·휠 클릭은 게임에도 그대로 전달됨
- 단축키·마우스 입력 기반 쿨타임 타이머와 스톱워치
  - 하나의 타이머에 여러 입력을 추가해 같은 타이머 시작 가능
